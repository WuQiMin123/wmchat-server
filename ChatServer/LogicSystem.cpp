...
void LogicSystem::LoginHandler(shared_ptr<CSession> session, const short &msg_id, const string &msg_data) {
	...
	//2.使用 Defer 实现延迟执行  在当前作用域结束时自动调用，把 rtvalue 转换为 JSON 字符串并发送给客户端
	Json::Value  rtvalue;
	...
	{
		//8.加锁处理登录状态（分布式锁）  此处添加分布式锁，让该线程独占登录
		//拼接用户ip对应的key
