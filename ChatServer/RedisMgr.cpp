...
bool RedisMgr::Get(const std::string& key, std::string& value)
{
	...
	if (reply == nullptr) {
		std::cout << "[ GET1  " << key << " ] failed: reply is null" << std::endl;
		_con_pool->returnConnection(connect);
		return false;
	}

	if (reply->type == REDIS_REPLY_NIL) {
		std::cout << "[ GET2  " << key << " ] key not found" << std::endl;
	...
	if (reply->type != REDIS_REPLY_STRING) {
		std::cout << "[ GET2  " << key << " ] unexpected type: " << reply->type
			<< ", message: " << (reply->str ? reply->str : "NULL") << std::endl;
		freeReplyObject(reply);
		_con_pool->returnConnection(connect);
		return false;
	}
