...
std::shared_ptr<PageResult> MysqlDao::LoadChatMsg(int thread_id, int last_message_id, int page_size)
{
	...
	try {
		...
		int count = 0; //
		// 读取 fetch_limit 条记录
		while (rs->next() && count < fetch_limit ) {
			ChatMessage msg;
		...
		if (count == fetch_limit) {//
			...
			// 通常，next_cursor 应该设置为最后一条（即多出来的那一条）的ID，但这条不应该返回给前端
			// 所以，我们移除最后一条，并将 next_cursor 设为它的 ID
			auto last_msg = page_res->messages.back();
			page_res->messages.pop_back(); // 移除多读取的那一条
			page_res->next_cursor = last_msg.message_id; // 设置下一页的起始ID
		}
		else {
			// 读取到的记录数少于 fetch_limit，说明已经没有更多数据了
			page_res->load_more = false;
			// 如果还有消息，next_cursor 可以设为最后一条消息的ID，或者保持为 last_message_id 也可以
			if (!page_res->messages.empty()) {
				page_res->next_cursor = page_res->messages.back().message_id;
			}
			// 如果没有消息，next_cursor 保持为传入的 last_message_id 或设为 0
		}
